...
def generate_one_point_data(N, beta, setting, rho_1, rho_2, sigma):   # 生成一个地点的数据
    ...
    if setting == 'discrete':
        Sigma = ar_cov_mat(p1 + p2 + p_noise, rho_1)
...
def generate_repeat_data(repeat_number, setting, rho_1, rho_2, sigma, signal_level):
    beta = np.full((l, p1 + p2 + 1), np.nan)  # 初始化回归系数
    a = np.linspace(1.5, 4, p1 + p2 + 1) * 10  # 得到不同的特征
    ...
    for feature in range(p1 + p2 + 1):  # 得到所有地点的回归系数
        ...
        elif signal_level == 'middle':
            beta[np.random.choice(l, 5 * l // 10, replace=False), feature] = beta[np.random.choice(l, 5 * l // 10, replace=False), feature] * 3
...
if __name__ == '__main__':
    ############ data generating ##############
    setting = 'continue'
    p1, p2, p_noise = 5, 0, 25  # number of continuous covariates, discrete covariates and noise covariates
    p, l, sample_average = p1 + p2 + p_noise, 10, 30  # number of covariates and number of locations
    np.random.seed(RANDOM_STATE)
    n = np.random.normal(sample_average, sample_average / 6, l).astype(int)  # 生成的样本不同样本取值不同
    # n = [sample_average for i in range(l)]
    print('setting=' + setting + ',  p1, p2, p_noise=' + str(p1) +','+ str(p2) +','+ str(p_noise) + ',  n=' + str(sample_average))

    repeat_number = 100
    rho_2, sigma = 0, 1/3
    RHO_1, lt, sl = [0, 0.2, 0.5], ['disperse'], ['low', 'high']
    for location_type in lt:
        ...
        for signal_level in sl:
            for rho_1 in RHO_1:
                # 保存不同设置下，每次模拟下的系数估计结果和整体的估计情况
                output_dir = 'SIM_100/' + 'setting=' + setting + ' location_type=' + location_type + ' signal_level=' \
                             + signal_level + ' p1, p2, p_noise=' + str(p1) + ',' + str(p2) + ',' + str(
                    p_noise) + ' rho_1, rho_2, sigma=' \
                             + str(rho_1) + ',' + str(rho_2) + ',' + str(sigma) + ',n=' + str(
                ...
                if not os.path.isdir(output_dir):
                    ...
                    os.mkdir(output_dir)
                repeat_data, beta, threshold = generate_repeat_data(repeat_number, setting, rho_1, rho_2, sigma, signal_level)  # 重复进行多次单地点实验，其中beta值保持不变，让X和误差项的值发生改变，从而改变y值
                ...
                for repeat in range(repeat_number):
                    ...
                    pd.DataFrame(np.concatenate((np.array(lsq_coef),np.array(lsq_intercept)), axis=1)).to_excel(output_dir2 +'lsq_coef' + ".xlsx")
                    pd.DataFrame(np.concatenate((np.array(weight_coef), np.array(weight_intercept)), axis=1)).to_excel(
                        output_dir2 +'weight_coef' + ".xlsx")
                    ...
                    pd.DataFrame(np.concatenate((np.array(weight_lasso_coef), np.array(weight_lasso_intercept).reshape(-1,1)), axis=1)).to_excel(
                        output_dir2 +'weight_lasso_coef' + ".xlsx")
                    pd.DataFrame(np.concatenate((np.array(simultaneous_model.coef_.reshape((l, p)))
                                                 , np.array(simultaneous_model.intercept_)), axis=1)).to_excel(output_dir2 +'sim_coef' + ".xlsx")
                ...
                print(round(selection_n4_average[0], 3), "(", round(selection_n4_std[0], 3), ")",
                      round(selection_n4_average[1], 3), "(", round(selection_n4_std[1], 3), ")",
                      round(selection_n4_average[2], 3), "(", round(selection_n4_std[2], 3), ")")
                print("lsq_lasso_fpr, weight_lasso_fpr, sim_fpr=", end='')
                print(round(selection_n2_average[0], 3), "(", round(selection_n2_std[0], 3), ")",
                      round(selection_n2_average[1], 3), "(", round(selection_n2_std[1], 3), ")",
                      round(selection_n2_average[2], 3), "(", round(selection_n2_std[2], 3), ")")
                ...
                result = {}
                result['lsq_average_loss(var)'] = [str(round(average[0], 3)) + ' (' + str(round(std[0], 3)) + ')']
                result['weight_average_loss(var)'] = [str(round(average[1], 3)) + ' (' + str(round(std[1], 3)) + ')']
                result['lasso_average_loss(var)'] = [str(round(average[2], 3)) + ' (' + str(round(std[2], 3)) + ')']
                result['weight_lasso_average_loss(var)'] = [str(round(average[3], 3)) + ' (' + str(round(std[3], 3)) + ')']
                result['simultaneous_average_loss(var)'] = [str(round(average[4], 3)) + ' (' + str(round(std[4], 3)) + ')']
                result['lsq_coef_average_loss(var)'] = [str(round(coef_average[0], 3)) + ' (' + str(round(coef_std[0], 3)) + ')']
                result['weight_coef_average_loss(var)'] = [str(round(coef_average[1], 3)) + ' (' + str(round(coef_std[1], 3)) + ')']
                result['lasso_coef_average_loss(var)'] = [str(round(coef_average[2], 3)) + ' (' + str(round(coef_std[2], 3)) + ')']
                result['weight_lasso_coef_average_loss(var)'] = [str(round(coef_average[3], 3)) + ' (' + str(round(coef_std[3], 3)) + ')']
                result['simultaneous_coef_average_loss(var)'] = [str(round(coef_average[4], 3)) + ' (' + str(round(coef_std[4], 3)) + ')']
                result['lasso_tpr(var)'] = [str(round(selection_n4_average[0], 3)) + ' (' + str(round(selection_n4_std[0], 3)) + ')']
                result['weight_lasso_tpr(var)'] = [str(round(selection_n4_average[1], 3)) + ' (' + str(round(selection_n4_std[1], 3)) + ')']
                result['sim_tpr(var)'] = [str(round(selection_n4_average[2], 3)) + ' (' + str(round(selection_n4_std[2], 3)) + ')']
                result['lasso_fpr(var)'] = [str(round(selection_n2_average[0], 3)) + ' (' + str(round(selection_n2_std[0], 3)) + ')']
                result['weight_lasso_fpr(var)'] = [str(round(selection_n2_average[1], 3)) + ' (' + str(round(selection_n2_std[1], 3)) + ')']
                result['sim_fpr(var)'] = [str(round(selection_n2_average[2], 3)) + ' (' + str(round(selection_n2_std[2], 3)) + ')']
                pd.DataFrame(result).to_excel(
                    output_dir + "Result, " + 'setting=' + setting + ' location_type=' + location_type + ' signal_level='
                    + signal_level + ' p1, p2, p_noise=' + str(p1) +','+ str(p2) +','+ str(p_noise) + ' rho_1, rho_2, sigma='
                    + str(rho_1) +','+ str(rho_2) +','+ str(sigma) + ',n=' + str(sample_average) + ',l=' + str(l) + ".xlsx")