...
def parallel_training(index, hyperparam_index, lamda, bw, g_coords):
    ...
    kfolder = KFold(n_splits=5, shuffle=True, random_state=RANDOM_STATE)
    for train_index, test_index in kfolder.split(X_train_o, y_train_o):  # 划分数据集为五份，进行五折交叉验证
        print('fold')
        X_train_x, X_test_c = X_train_o[train_index], X_train_o[test_index]  # 训练集对应的值
        y_train_x, y_test_c = y_train_o[train_index], y_train_o[test_index]  # 验证集对应的值
...
if __name__ == '__main__':
    ...
    split1 = StratifiedShuffleSplit(n_splits=1, test_size=0.2, random_state=RANDOM_STATE)
    ...
    X_train_o, X_test_o, y_train_o, y_test_o = np.array(X_train_o), np.array(X_test_o), np.array(y_train_o),np.array(y_test_o)
    ...
    simultaneous_model.fit(X_train_o, y_train_o, Aerfa, g_coords, p, l)
    ...
    for loc in range(len(g_coords)):
        ...
        weight_lasso_model.fit(X_train_o, y_train_o, g_coords, loc, aerfa)
    ...
    for loc in range(len(g_coords)):
        ...
        #########################进行单地点最小二乘回归###################################
        lsq_model = one_point_estimation(add_weight=False, lamd=0, tol=1e-10, max_iter=1000)
        lsq_model.fit(X_train_o, y_train_o, g_coords, loc, aerfa)
        ...
        weight_model.fit(X_train_o, y_train_o, g_coords, loc, aerfa)
        ...
        lasso_model = one_point_estimation(add_weight=False, lamd=lamda[np.argmin(loss_par[2, :]) // len(bw)], tol=1e-10,
                                           max_iter=1000)
        lasso_model.fit(X_train_o, y_train_o, g_coords, loc, aerfa)
